...
while (( $n < $nend )) ; do
  ...
  while (( $seed < $ninstances )) ; do
    ...
    ub0=`cat toolbar_opt`
    ub=`expr $ub0 + 2`
    mpirun -n 4 ./toulbar2 problem.wcsp "$@" -w -ub=$ub | awk 'BEGIN{opt="-";} /Read [0-9]* variables/{n=$2;} / unassigned variables/{f=$1} /^Optimum: /{opt=$2;}  END{printf("%d %d %d",opt,n,f);}'  > toulbar2_opt
