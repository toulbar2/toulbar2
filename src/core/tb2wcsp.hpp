...
public:
    ...
    Double getDDualBound() const { return Cost2ADCost(lb); } ///< \brief gets problem primal bound as a Double representing a decimal cost (upper resp. lower bound for minimization resp. maximization)
    Double getDPrimalBound() const { return Cost2ADCost(ub); } ///< \brief gets problem dual bound as a Double representing a decimal cost (lower resp. upper bound for minimization resp. maximization)
