...
public:
        ...
        vector<pair<Value, Value>>& supportX)
    {
        ...
        if (y->cannotbe(ysupport) || z->cannotbe(zsupport) || getCostWithBinaries(x, y, z, x->getSupport(), ysupport, zsupport) + y->getCost(ysupport) + z->getCost(zsupport) > MIN_COST || ((ToulBar2::vacValueHeuristic & VAC_SUPPORT_HEUR) && Store::getDepth() < abs(ToulBar2::vac))) {
            x->queueEAC2();
        }
    }
