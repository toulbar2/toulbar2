    ...
    void fillEAC2(int varIndex)
    {
        ...
        if (getDACScopeIndex() == 0) {
            if (varIndex == 0) {
                ...
                if (x->cannotbe(supportY[yindex]) || x->getCost(supportY[yindex]) > MIN_COST || getCost(supportY[yindex], y->getSupport()) > MIN_COST || ((ToulBar2::vacValueHeuristic & VAC_SUPPORT_HEUR) && Store::getDepth() < abs(ToulBar2::vac))) {
                    y->queueEAC2();
                }
            }
        } else {
            if (varIndex == 1) {
                ...
                if (y->cannotbe(supportX[xindex]) || y->getCost(supportX[xindex]) > MIN_COST || getCost(x->getSupport(), supportX[xindex]) > MIN_COST || ((ToulBar2::vacValueHeuristic & VAC_SUPPORT_HEUR) && Store::getDepth() < abs(ToulBar2::vac))) {
                    x->queueEAC2();
                }
            }
        }
