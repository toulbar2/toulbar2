...
bool VNSSolver::solve()
{
    ...
    if (ToulBar2::verbose >= 1)
        cout << "VNS: initial solution with" << ((complete) ? " optimal" : "") << " cost " << bestUb << endl;
    try {
    ...
    while (!stop && !complete && bestUb > ToulBar2::vnsOptimum) {
        if (ToulBar2::verbose >= 0 && ToulBar2::restart > 1 && ToulBar2::lds)
            cout << "****** Restart " << nbRestart << " with " << lds << " discrepancies and UB=" << bestUb << " ****** (" << nbNodes << " nodes)" << endl;
        Long rank = 1;
        ...
        while (!complete && k <= ToulBar2::vnsKmax && bestUb > ToulBar2::vnsOptimum) {
            ...
            if (lastUb >= bestUb) {
                ...
                if (ToulBar2::verbose >= 1)
                    cout << "VNS: new solution with cost " << bestUb << endl;
            }
        }
