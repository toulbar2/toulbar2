...
bool VNSSolver::solve(bool first)
{
    ...
    try {
        ...
        if (ToulBar2::verbose >= 0 && ToulBar2::vnsNeighborVarHeur == CLUSTERRAND && ((ClustersNeighborhoodStructure*)h)->getSize() > 1) {
            ...
            cout << "Problem decomposition in (test) " << ch->getSize() << " clusters with size distribution: min: " << ch->getMinClusterSize() << " median: " << ch->getMedianClusterSize() << " mean: " << ch->getMeanClusterSize() << " max: " << ch->getMaxClusterSize() << endl;
        }
        // vns/lds+cp
        Long nbRestart = 1;
        Long restart = 1;
