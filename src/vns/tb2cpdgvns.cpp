...
bool CooperativeParallelDGVNS::solveLS()
{
    ...
    while (finished < worker) {
        ...
        if (sbestUb <= bestUb
            ...
            if (ToulBar2::vnsOutput) {
                ...
                ToulBar2::vnsOutput << "Solution " << " (" << sk << " "
                                    << scluster << ") ";
                for (map<int, Value>::iterator it = bestSolution.begin();
                        it != bestSolution.end(); ++it)
...
void CooperativeParallelDGVNS::slave()
{
    ...
    wcsp->setUb(MAX_COST);
    initVarHeuristic();
    // loop for getting new initint ial solution to initialize the search process
    ...
    while (1) {
        ...
        MPI_Send(&env0.sendbuff[0], env0.buffsize, MPI_INT, 0, 0,
                MPI_COMM_WORLD);
...
void CooperativeParallelDGVNS::VnsLdsCP(MPIEnv &env0, double btime, ParallelRandomClusterChoice* h)
{
    ...
    for (; k <= kmax && k <= (int) wcsp->numberOfVariables() && ToulBar2::vnsOptimum < bestUb;) {
        ...
        //repair
        if (ToulBar2::lds) repair_recursiveSolve(ToulBar2::lds, variables, values, bestUb);
        else repair_recursiveSolve(variables, values, bestUb);
        //updating
