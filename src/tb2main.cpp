...
CSimpleOpt::SOption g_rgOptions[] =
{
    ...
    { OPT_VNS_search, (char*) "--vns", SO_NONE  },
    { OPT_DGVNS_search, (char*) "-dgvns", SO_NONE  },
    { OPT_DGVNS_search, (char*) "--dgvns", SO_NONE  },
#ifdef OPENMPI
...
//    { OPT_vns_var_heur, (char*) "--variable-heuristic", SO_REQ_SEP },
    ...
    { OPT_optimum, (char*) "--best", SO_REQ_SEP },
	{ OPT_neighb_algo,          (char*) "-neighb_algo",             SO_REQ_SEP }, 
	{ OPT_root_algo,      		(char*) "-root_algo",               SO_REQ_SEP }, 
	{ OPT_atom_type,            (char*) "-atom_type",             	SO_REQ_SEP }, 
	{ OPT_forbiden_AA,          (char*) "-forbiden_AA",             SO_REQ_SEP }, 
	{ OPT_vns_verbosity_lvl,    (char*) "-vns_display_lvl",         SO_REQ_SEP }, 
	{ OPT_kpp_algo,             (char*) "-kpp_algo",                SO_REQ_SEP },  
    SO_END_OF_OPTIONS };
...
void help_msg(char *toulbar2filename)
{
    ...
    cout << "   *.pdb : pdb file for random selection of neighborhoods based on euclidean distances between amino acid in vns" << endl << endl;
	...
	cout << " 	-seed [interger] : initialize random generators in vns with this seed + id_mpi_thread (default use timer + pid)." << endl; 
	cout << "	-kpp_algo [integer] : value to select the algorithm associated with the increase of k values (default is basic ++k) :" << endl;
	cout << "			= 0 -> basic ++k." << endl;
	cout << " 			= 1 -> ++k with Luby." << endl;
	cout << " 			= 2 -> ++k keep the last neighborhood as base to build a new one." << endl;
	cout << "	-root_algo [integer] : value to select the algorithm (default full random and uniform) which select the first element in a neighborhood :" << endl;
	cout << " 			= 0 -> random and uniform." << endl;
	cout << "  			= 1 -> centre/periphery (use *.pdb parameter)." << endl;
	cout << "	-neighb_algo [integer] : value to select the algorithm (default uniform shuffle) which builds neighborhoods :" << endl;
	cout << "			= 0 -> shuffle." << endl; 
	cout << " 			= 1 -> 1/d proba (use *.pdb parameter)." << endl; 
	cout << "			= 2 -> k nearest (use *.pdb parameter)." << endl; 
	cout << " 	-atom_type [string] : use this atom type (CA,CB,..) to compute the distance matrix for vns (default CA)." << endl;
	cout << " 	-forbiden_AA [string] : to read pdb file : provide a list (format = \"PRO:GLY:...\") of amino acids which will not be used as variables (default \"GLY:PRO\")." << endl;
	cout << "	-vns_display_lvl [integer] : controle the messages print on sreen : 0,1,2,.. 0 is a few and 2 is a lot." << endl;
#ifndef MENDELSOFT
    ...
    cout << endl;
    cout << "   -L=[integer] : randomized (quasi-random variable ordering) search with restart (maximum number of nodes/VNS = " << maxrestarts << " by default)";
    ...
    cout << "   -vns : initial upperbound found by variable neighborhood search" << endl;
    cout << "   -dgvns : initial upperbound found by decomposition guided VNS (see *.dec file description)" << endl; //TODO: use -B=1 option instead
#ifdef OPENMPI
...
//    cout << "   -rsdgvns : initial upperbound found by replicated synchronous DGVNS (usage: \"mpirun -n [NbOfProcess] toulbar2 -rsdgvns problem.wcsp\")" << endl;
    cout << "   -radgvns : initial upperbound found by replicated asynchronous DGVNS (usage: \"mpirun -n [NbOfProcess] toulbar2 -radgvns problem.wcsp\")" << endl;
#endif
    cout << "   -ldsini=[integer] : initial solution for VNS-like methods found (-1) at random, (-2) min domain values, (-3) max domain values, (-4) first solution found by a complete method, (k=0 or more) tree search with k discrepancy max (" << ToulBar2::vnsInitSol <<  " by default)" << endl;
    cout << "   -ldsmin=[integer] : minimum discrepancy for VNS-like methods (" << ToulBar2::vnsLDSmin <<  " by default)" << endl;
    cout << "   -ldsmax=[integer] : maximum discrepancy for VNS-like methods (" << ToulBar2::vnsLDSmax <<  " by default)" << endl;
    ...
    cout << "   -kinc=[integer] : neighborhood size increment strategy for VNS-like methods using (1) Add1, (2) Mult2, (3) Luby operator (" << ToulBar2::vnsKinc <<  " by default)" << endl;
...
typedef enum SearchMethod {
    COMPLETE, VNS, DGVNS, CPDGVNS, RPDGVNS, TREEDEC
} SearchMethod;
...
int _tmain(int argc, TCHAR * argv[])
{
    ...
    MPI_Comm_split(MPI_COMM_WORLD, env0.myrank == 0, 0, &env0.new_comm);
#endif
    tb2init();
    ...
    SearchMethod searchMethod = COMPLETE;
    ...
    file_extension_map["pdb_ext"]=".pdb"; 

    assert(cout << "Warning! toulbar2 was compiled in debug mode and it can be very slow..." << endl);
    if (ToulBar2::verbose >= 0) cout << "c " << CurrentBinaryPath<<"toulbar2"<<"  version : " << ToulBar2::version << ", copyright (c) INRA 2016"<<endl;
    ...
    while (args.Next())
    {
        ...
        if (args.LastError() == SO_SUCCESS)
        {
            ...
            if (args.OptionId() == OPT_DGVNS_search) {
                searchMethod = DGVNS;
                ToulBar2::vnsNeighborVarHeur = CLUSTERRAND;
            }
			if (args.OptionId() == OPT_neighb_algo) {
				if (args.OptionArg() != NULL) {
					ToulBar2::random_selection_neighborhood = atoi(args.OptionArg()); 
				}
			}
			if (args.OptionId() == OPT_root_algo) {
				if (args.OptionArg() != NULL) {
					ToulBar2::random_selection_first_element = atoi(args.OptionArg()); 
				}
			}
			if (args.OptionId() == OPT_atom_type) {
				if (args.OptionArg() != NULL) {
					ToulBar2::atom_type = args.OptionArg(); 
				}
			}
			if (args.OptionId() == OPT_forbiden_AA) {
				if (args.OptionArg() != NULL) {
					ToulBar2::forbiden_AA = args.OptionArg(); 
				}
			}
			if (args.OptionId() == OPT_vns_verbosity_lvl) {
				if (args.OptionArg() != NULL) {
					ToulBar2::vns_verbosity_level = atoi(args.OptionArg()); 
				}
			}
			if (args.OptionId() == OPT_kpp_algo) {	
				if (args.OptionArg() != NULL) {
					ToulBar2::incr_k_algo = atoi(args.OptionArg());
				} 
			}
#ifdef OPENMPI
            if (args.OptionId() == OPT_CPDGVNS_search) {
                searchMethod = CPDGVNS;
                ToulBar2::vnsParallel = true;
                ToulBar2::vnsNeighborVarHeur = MASTERCLUSTERRAND;
            }
            if (args.OptionId() == OPT_RADGVNS_search) {
                searchMethod = RPDGVNS;
                ToulBar2::vnsParallel = true;
                ToulBar2::vnsNeighborVarHeur = MASTERCLUSTERRAND;
...
//            }
    ...
    if(random_desc ==NULL)
    {
        for (int n = 0; n < glob.FileCount(); ++n) {
			...
			if(check_file_ext(glob.File(n), file_extension_map["pdb_ext"]) ) 
			{
				//strext = ".pdb";
				//strfile = glob.File(n);
				cout << "Compute distance matrix from file : " << glob.File(n) << " with " << ToulBar2::atom_type << endl;
				// I. Extract CA or CB from pdb file
				ToulBar2::AA_vector = extract_pdb(glob.File(n), ToulBar2::atom_type, ToulBar2::forbiden_AA);
				// II. Compute the centroid
				ToulBar2::AA_centroid = compute_centroid(ToulBar2::AA_vector);
				cout << ToulBar2::atom_type << "' centroid = x:" << ToulBar2::AA_centroid->x 
					<< " y:" << ToulBar2::AA_centroid->y 
					<< " z:" << ToulBar2::AA_centroid->z
					<< " distCO:" <<  ToulBar2::AA_centroid->distance()
					<< endl;	
				// III. Compute distances between CA atoms
				ToulBar2::distance_matrix = compute_distances(ToulBar2::AA_vector, ToulBar2::AA_centroid);
				ToulBar2::check_edrsn = true;
			}
    ...
    case TREEDEC:
        ...
        break;
    default:
        solver = WeightedCSPSolver::makeWeightedCSPSolver(ub);
        break;
    };
    ...
    try
    {
        if (randomproblem)    solver->read_random(n,m,p,ToulBar2::seed,forceSubModular,randomglobal);
...
#endif
            if (searchMethod == COMPLETE) {
                solver->solve();
            } else {
                if (!ToulBar2::lds || ToulBar2::vnsLDSmax < 0) ToulBar2::vnsLDSmax = solver->getWCSP()->numberOfVariables() * (solver->getWCSP()->getMaxDomainSize()-1);
                if (!ToulBar2::lds) ToulBar2::vnsLDSmin = solver->getWCSP()->numberOfVariables() * (solver->getWCSP()->getMaxDomainSize()-1);
                if (ToulBar2::vnsKmax <= 0) ToulBar2::vnsKmax = solver->getWCSP()->numberOfVariables();
                if (ToulBar2::restart < 1) ToulBar2::restart = 1; // Force random variable selection during (LDS) search
                static_cast<LocalSearch*>(solver)->solveLS();
            }
        }
    }
    catch (Contradiction)
	...
	// clean data 
	// AA_vector
	for (unsigned int i = 0; i < ToulBar2::AA_vector->size(); ++i) {
		delete (* ToulBar2::AA_vector)[i];
		(* ToulBar2::AA_vector)[i] = NULL;
	}
	ToulBar2::AA_vector->clear();
	delete ToulBar2::AA_vector;
	ToulBar2::AA_vector = NULL;
	// distance_matrix	
	for (unsigned int i = 0; i < ToulBar2::distance_matrix->size(); ++i) {
		(* ToulBar2::distance_matrix)[i].clear();
	}
	ToulBar2::distance_matrix->clear();
	delete ToulBar2::distance_matrix;
	ToulBar2::distance_matrix = NULL;
	// centroid
	delete ToulBar2::AA_centroid;
	ToulBar2::AA_centroid = NULL;
