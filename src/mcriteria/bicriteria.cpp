...
std::mutex Bicriteria::_interruption_mutex;
bool Bicriteria::_global_interruption = false;
...
void Bicriteria::computeSupportedPoints(MultiCFN* multicfn, unsigned int first_cfn_index, unsigned int second_cfn_index, pair<Bicriteria::OptimDir, Bicriteria::OptimDir> optim_dir, Double delta)
{
    ...
    _global_interruption = false;

    // set the global timer
    if(_global_timeout > 0) {
        Bicriteria::global_timer(_global_timeout);
    }
    ...
    while (!pending.empty()) {
        ...
        // or if global timeout triggered
        if(Bicriteria::_global_interruption || (Bicriteria::_nsol_max > 0 && static_cast<int>(_solutions.size()) >= Bicriteria::_nsol_max) ) {
            break;
        }
