...
// Matrix of distances between molecule's amino acids
vector<vector<double> > * ToulBar2::distance_matrix = NULL; 
// Type of atom used to compute distances between amino acids
string ToulBar2::atom_type = "CB";
// List of forbiden amino acids to compute the matrix of distances
// Format : "GLY:PRO: ..." 
string ToulBar2::forbiden_AA = "GLY:PRO";
// Vector of pdb's data concerning molecule's amino acids
vector<tb2_amino_acid *> * ToulBar2::AA_vector = NULL; 
// Value to select the algorithm (default uniform shuffle) which builds neighborhoods
// 0 for shuffle
// 1 for 1/d proba
// 2 for k nearest
int ToulBar2::random_selection_neighborhood = 0; 
// Value to select the algorithm (default full random and uniform) which select the first element in a neighborhood
// 0 random
// 1 centre/periphery
int ToulBar2::random_selection_first_element = 0;
// Molecule's barycentre computed with all positions of amino acids 
tb2_point * ToulBar2::AA_centroid = NULL; 
// Value to choose the display level of data concerning neighborhoods
int ToulBar2::vns_verbosity_level = 0;
// Boolean to verify that the distance matrix, the molecule's centroid and the AA_vector have been previously computed 
// for the algorithms wich use these data
bool ToulBar2::check_edrsn = false;
// Value to select the algorithm associated with the increase of k values
// 0 basic ++k
// 1 ++k with Luby
// 2 ++k keep the last neighborhood as base to build a new one
int ToulBar2::incr_k_algo = 0;
// if ture -> use last neighborhood to build the new one, 
// add missing elements to the last neighborhood to provide the new neighborhood
// if false -> build new neighborhood regardless the last one
...
void WCSP::sortConstraints()
{
    ...
    if (ToulBar2::varOrder && (numberOfVariables() < LARGE_NB_VARS || ((long)((void *) ToulBar2::varOrder)) < 2 || ((long)((void *) ToulBar2::varOrder)) > 6)) {
        vector<int> order;
...
void WCSP::preprocessing() {
    ...
    if (ToulBar2::varOrder && numberOfVariables() >= LARGE_NB_VARS && numberOfUnassignedVariables() < LARGE_NB_VARS && (((long)((void *) ToulBar2::varOrder)) >= 2 && ((long)((void *) ToulBar2::varOrder)) <= 6)) {
        vector<int> order;
        if (isAlreadyTreeDec(ToulBar2::varOrder)) treeDecFile2Vector(ToulBar2::varOrder, order);
        else elimOrderFile2Vector(ToulBar2::varOrder, order);
        setDACOrder(order);
...
void WCSP::propagateNC() {
    if (ToulBar2::verbose >= 2) cout << "NCQueue size: " << NC.getSize() << " (" << NCBucketSize << " buckets maxi)" << endl;
    while (!NC.empty()) {
        Variable *x = NC.pop();
    ...
    if (objectiveChanged) {
        objectiveChanged = false;
        int bucket = min(cost2log2glb(getUb() - getLb()), NCBucketSize-1);
