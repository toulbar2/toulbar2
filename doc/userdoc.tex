...
\item knapsack {\itshape capacity} ({\itshape weight)$\ast$ to express a reverse knapsack hard constraint with minimum capacity and a list of weights associated to the variables in the scope of the constraint (warning! it assumes Boolean 0/1 variables and is equivalent to a linear constraint with >= operator, use negative numbers to express <=)
\end{DoxyItemize}
