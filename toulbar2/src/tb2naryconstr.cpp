        ...
        nonassigned(arity_in)
{
    ...
    for (i = 0; i < arity_in; i++) {
        ...
        tbuf[i] = CHAR_FIRST;
        if(domsize + CHAR_FIRST > std::numeric_limits<Char>::max()) {
...
NaryConstraint::NaryConstraint(WCSP *wcsp) : AbstractNaryConstraint(wcsp), nonassigned(0)
{
}
...
void Trie::insert(Char *word, Cost c)
{
    ...
    while (true) {
        ...
        if ((int)pos == notFound) {          								 // by the first letter of word
            ...
            if ((int)Strlen(lf->word) > offset + 1) s = lf->word + offset + 1;
            else s = &emptystr[0];
